...
const SendForm = ({active, socket, receiver}) => {
  ...
  useEffect(() => {
    socket.onmessage = (event) => {
      ...
      if (data.type === 'message') {
        ...
        const inThisChat =
          (msg.from === receiverId && msg.to === myId) || (msg.from === myId && msg.to === receiverId);
        if (inThisChat) {
          addMessages([msg]);
        }
  ...
  return (
    <div
      className={
        active
          ? 'flex flex-col flex-auto h-screen form active pl-4'
          : 'flex flex-col flex-auto h-screen form pl-4'
      }
    >
